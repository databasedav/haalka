//! Fixed version (AUTOMATED) - direct spawn/despawn in on_click (no child_signal)

mod utils;
use utils::examples_plugin;

use bevy::{app::AppExit, prelude::*, ui::Pressed};
        ...
        el.lazy_entity(lazy_entity.clone())
            ...
            .child(
                Button::new()
                    ...
                    .on_click(clone!((lazy_entity, opts_for_list) move |_: In<_>, world: &mut World| {
                        let frame = world.resource::<FrameCounter>().0;
                        if world.entity(*lazy_entity).contains::<DropdownShowing>() {
                            // Close this dropdown - despawn options container
                            info!("[DIRECT_SPAWN][Frame {}] UPDATE on_click: closing dropdown {:?}", frame, *lazy_entity);
                            ...
                            let container = world
                                ...
                                .iter(world)
                                .find(|&e| world.get::<DropdownOptionsContainer>(e).map(|c| c.owner == *lazy_entity).unwrap_or(false));
                            if let Some(container) = container {
                                info!("[DIRECT_SPAWN][Frame {}] UPDATE despawning options container {:?} for dropdown {:?}", frame, container, *lazy_entity);
                                world.entity_mut(container).despawn();
                            }
                        } else {
                            // Close any other open dropdowns first
                            let other_containers: Vec<Entity> = world
                                .query_filtered::<Entity, With<DropdownOptionsContainer>>()
                                .iter(world)
                                .collect();
                            for container in other_containers {
                                let owner = world.get::<DropdownOptionsContainer>(container).unwrap().owner;
                                info!("[DIRECT_SPAWN][Frame {}] UPDATE despawning other options container {:?} (owner {:?})", frame, container, owner);
                                world.entity_mut(owner).remove::<DropdownShowing>();
                                world.entity_mut(container).despawn();
                            }
                            
                            // Open this dropdown - spawn options container directly
                            info!("[DIRECT_SPAWN][Frame {}] UPDATE on_click: opening dropdown {:?}", frame, *lazy_entity);
                            world.entity_mut(*lazy_entity).insert(DropdownShowing);
                            
                            // Build and spawn the options column directly
                            info!("[DIRECT_SPAWN][Frame {}] UPDATE spawning options container for dropdown {:?}", frame, *lazy_entity);
                            let options_column = Column::<Node>::new()
                                .insert(DropdownOptionsContainer { owner: *lazy_entity })
                                .with_node(|mut node| {
                                    node.width = Val::Percent(100.0);
                                ...
                                .items(
                                    ...
                                    .map(clone!((lazy_entity) move |(i, opt)| {
                                        text_button(
                                            ...
                                            clone!((lazy_entity) move |_: In<_>, world: &mut World| {
                                                world.get_mut::<DropdownSelectionIndex>(*lazy_entity).unwrap().0 = Some(i);
                                                world.entity_mut(*lazy_entity).remove::<DropdownShowing>();
                                                // Find and despawn our container
                                                let container = world
                                                    .query_filtered::<Entity, With<DropdownOptionsContainer>>()
                                                    .iter(world)
                                                    .find(|&e| world.get::<DropdownOptionsContainer>(e).map(|c| c.owner == *lazy_entity).unwrap_or(false));
                            ...
                            info!("[DIRECT_SPAWN][Frame {}] UPDATE spawned options container {:?} for dropdown {:?}", frame, child_entity, *lazy_entity);
                        }
                    })),
            )
    }
...
fn ui_root(entities: &DropdownEntities) -> El<Node> {
    ...
    El::<Node>::new()
        ...
        .align_content(Align::center())
        .child(Column::<Node>::new().items([dropdown_one, dropdown_two]))
}
...
fn auto_clicker(world: &mut World) {
    let frame = world.resource::<FrameCounter>().0;
    let entities = world.resource::<DropdownEntities>();
    let second = *entities.second;
    
    // Frame 10: Open second dropdown using direct spawn approach
    if frame == 10 {
        info!("[DIRECT_SPAWN][Frame {}] ===== AUTO-CLICK: Opening second dropdown =====", frame);
        
        // This simulates the on_click handler being called
        if !world.entity(second).contains::<DropdownShowing>() {
            world.entity_mut(second).insert(DropdownShowing);
            
            // Build and spawn the options column directly (simulating what on_click does)
            let opts_for_list = vec![
                "One".to_string(),
                "Two".to_string(),
                "Three".to_string(),
                "Four".to_string(),
            ...
            info!("[DIRECT_SPAWN][Frame {}] UPDATE spawning options container for dropdown {:?}", frame, second);
            
            let option_buttons: Vec<El<Node>> = opts_for_list.iter().enumerate().map(|(_i, opt)| {
                El::<Node>::new()
                    .with_node(|mut node| {
                        node.width = Val::Percent(100.0);
                        node.height = Val::Px(DEFAULT_BUTTON_HEIGHT);
                    ...
                    .child(
                        El::<Text>::new()
                            .text(Text::new(opt.clone()))
                            .text_font(TextFont::from_font_size(FONT_SIZE))
                    )
            }).collect();
            
            let options_column = Column::<Node>::new()
                .insert(DropdownOptionsContainer { owner: second })
                .with_node(|mut node| {
                    node.width = Val::Percent(100.0);
                    node.top = Val::Percent(100.0);
                    node.position_type = PositionType::Absolute;
            ...
            let child_entity = world.spawn_empty().id();
            world.entity_mut(second).add_child(child_entity);
            options_column.into_builder().spawn_on_entity(world, child_entity).unwrap();
            info!("[DIRECT_SPAWN][Frame {}] UPDATE spawned options container {:?} for dropdown {:?}", frame, child_entity, second);
        }
    }
    
    // Exit after frame 15 to give time to observe
    if frame == 15 {
        info!("[DIRECT_SPAWN][Frame {}] ===== AUTO-EXIT =====", frame);
        std::process::exit(0);
    }
}

fn log_entity_state(world: &mut World) {
    let frame = world.resource::<FrameCounter>().0;
    let entities = world.resource::<DropdownEntities>();
    let second = *entities.second;
    ...
    if second_showing {
        let second_children: Vec<Entity> = world.get::<Children>(second)
            .map(|c| c.iter().collect())
            .unwrap_or_default();
        
        info!("[DIRECT_SPAWN][Frame {}] FIRST: second dropdown has {} children", frame, second_children.len());
        
        if second_children.len() > 1 {
            let options = second_children[1];
            let exists = world.get_entity(options).is_ok();
            let has_computed_node = world.entity(options).get::<bevy::ui::ComputedNode>().is_some();
            let has_ui_global_t = world.entity(options).get::<bevy::ui::UiGlobalTransform>().is_some();
            let has_visibility = world.entity(options).get::<InheritedVisibility>().is_some();
            let has_bg = world.entity(options).get::<BackgroundColor>().is_some();
            
            info!("[DIRECT_SPAWN][Frame {}] FIRST: options entity exists={} ComputedNode={} UiGlobalTransform={} InheritedVisibility={} BackgroundColor={}", 
                frame, exists, has_computed_node, has_ui_global_t, has_visibility, has_bg);
        }
    }
}

fn log_entity_state_last(world: &mut World) {
    let frame = world.resource::<FrameCounter>().0;
    let entities = world.resource::<DropdownEntities>();
    let second = *entities.second;
    ...
    if second_showing {
        ...
        info!("[DIRECT_SPAWN][Frame {}] LAST: second dropdown has {} children", frame, second_children.len());
        ...
        if second_children.len() > 1 {
            ...
            info!("[DIRECT_SPAWN][Frame {}] LAST: options entity exists={} ComputedNode={} UiGlobalTransform={} InheritedVisibility={} BackgroundColor={}", 
                frame, exists, has_computed_node, has_ui_global_t, has_visibility, has_bg);
        }
    }
}
...
fn main() {
    App::new()
        ...
        .add_systems(First, log_entity_state.after(increment_frame_counter))
        .add_systems(Update, auto_clicker)
        .add_systems(Last, log_entity_state_last)
