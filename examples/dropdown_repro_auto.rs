//! Automated repro for dropdown one-frame delay - no user interaction needed
    ...
    fn into_el(self) -> Self::EL {
        ...
        let show = signal::from_entity(lazy_entity.clone())
            ...
            .map(clone!((lazy_entity) move |In(showing): In<bool>, counter: Res<FrameCounter>| {
                info!("[SIGNAL_CHILD][Frame {}] show signal fired for {:?}: {}", counter.0, *lazy_entity, showing);
                showing
            }));
        ...
        el.lazy_entity(lazy_entity.clone())
                ...
                Button::new()
                    ...
                    .on_click(clone!((lazy_entity) move |_: In<_>, world: &mut World| {
                        ...
                        if world.entity(*lazy_entity).contains::<DropdownShowing>() {
                            info!("[SIGNAL_CHILD][Frame {}] UPDATE on_click: removing DropdownShowing from {:?}", frame, *lazy_entity);
                            ...
                            for entity in world.query_filtered::<Entity, With<DropdownShowing>>().iter(world).collect::<Vec<_>>() {
                                info!("[SIGNAL_CHILD][Frame {}] UPDATE on_click: removing DropdownShowing from {:?} (closing other)", frame, entity);
                                world.entity_mut(entity).remove::<DropdownShowing>();
                            }
                            info!("[SIGNAL_CHILD][Frame {}] UPDATE on_click: adding DropdownShowing to {:?}", frame, *lazy_entity);
            ...
            .child_signal(
                show.map_true_in(clone!((lazy_entity, opts_for_list) move || {
                    info!("[SIGNAL_CHILD] map_true_in: creating Column for {:?}", *lazy_entity);
                    Column::<Node>::new()
                        .with_node(|mut node| {
                            node.width = Val::Percent(100.0);
...
fn auto_clicker(world: &mut World) {
    ...
    // Frame 10: Open second dropdown
    if frame == 10 {
        info!("[SIGNAL_CHILD][Frame {}] ===== AUTO-CLICK: Opening second dropdown =====", frame);
        world.entity_mut(second).insert(DropdownShowing);
    }
    
    // Exit after frame 15 to give time to observe
    if frame == 15 {
        info!("[SIGNAL_CHILD][Frame {}] ===== AUTO-EXIT =====", frame);
        std::process::exit(0);
    }
}
...
fn log_entity_state(world: &mut World) {
    ...
    if second_showing {
        ...
        info!("[SIGNAL_CHILD][Frame {}] FIRST: second dropdown has {} children", frame, second_children.len());
        ...
        if second_children.len() > 1 {
            ...
            info!("[SIGNAL_CHILD][Frame {}] FIRST: options entity exists={} ComputedNode={} UiGlobalTransform={} InheritedVisibility={} BackgroundColor={}", 
...
fn log_entity_state_last(world: &mut World) {
    ...
    if second_showing {
        ...
        info!("[SIGNAL_CHILD][Frame {}] LAST: second dropdown has {} children", frame, second_children.len());
        ...
        if second_children.len() > 1 {
            ...
            info!("[SIGNAL_CHILD][Frame {}] LAST: options entity exists={} ComputedNode={} UiGlobalTransform={} InheritedVisibility={} BackgroundColor={}", 
                frame, exists, has_computed_node, has_ui_global_t, has_visibility, has_bg);
        }
    }
}
