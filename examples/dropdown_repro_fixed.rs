//! Fixed version - direct spawn/despawn in on_click (no child_signal)
                ...
                Button::new()
                    ...
                    .on_click(clone!((lazy_entity, opts_for_list) move |_: In<_>, world: &mut World| {
                        ...
                        if world.entity(*lazy_entity).contains::<DropdownShowing>() {
                            ...
                            info!("[FIXED][Frame {}] on_click: closing dropdown {:?}", frame, *lazy_entity);
                            ...
                            if let Some(container) = container {
                                info!("[FIXED][Frame {}] despawning options container {:?} for dropdown {:?}", frame, container, *lazy_entity);
                            ...
                            for container in other_containers {
                                ...
                                info!("[FIXED][Frame {}] despawning other options container {:?} (owner {:?})", frame, container, owner);
                                world.entity_mut(owner).remove::<DropdownShowing>();
                                world.entity_mut(container).despawn();
                            }
                            
                            // Open this dropdown - spawn options container directly
                            info!("[FIXED][Frame {}] on_click: opening dropdown {:?}", frame, *lazy_entity);
                            world.entity_mut(*lazy_entity).insert(DropdownShowing);
                            
                            // Build and spawn the options column directly
                            info!("[FIXED][Frame {}] spawning options container for dropdown {:?}", frame, *lazy_entity);
                            ...
                            info!("[FIXED][Frame {}] spawned options container {:?} for dropdown {:?}", frame, child_entity, *lazy_entity);
                        }
                    })),
            )
    }
        ...
        .add_systems(First, (|world: &mut World| {
            ...
            if second_showing {
                ...
                if second_children.len() > 1 {
                    ...
                    info!("[FIXED][Frame {}] START: options_has_UiGlobalTransform={}", 
        ...
        .add_systems(Last, |world: &mut World| {
            ...
            if second_showing {
                ...
                if second_children.len() > 1 {
                    ...
                    info!("[FIXED][Frame {}] END: options_has_UiGlobalTransform={}", 
                        frame, has_ui_global_t);
                }
            }
        })
