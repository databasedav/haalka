//! Diagnostic tool to trace exact timing of dropdown spawn/despawn operations
//! Run this to see the exact sequence of events and identify the flicker cause
    ...
    fn into_el(self) -> Self::EL {
        ...
        let show = signal::from_entity(lazy_entity.clone())
            ...
            .map(clone!((lazy_entity) move |In(showing): In<bool>, counter: Res<FrameCounter>| {
                info!("[TIMING][Frame {}] SIGNAL: show signal fired for {:?}: {}", counter.0, *lazy_entity, showing);
                showing
            }));
            ...
            .child(
                Button::new()
                    ...
                    .on_click(clone!((lazy_entity) move |_: In<_>, world: &mut World| {
                        ...
                        info!("[TIMING][Frame {}] UPDATE: on_click handler START for {:?}", frame, *lazy_entity);
                        if world.entity(*lazy_entity).contains::<DropdownShowing>() {
                            info!("[TIMING][Frame {}] UPDATE: removing DropdownShowing from {:?}", frame, *lazy_entity);
                            ...
                            for entity in world.query_filtered::<Entity, With<DropdownShowing>>().iter(world).collect::<Vec<_>>() {
                                info!("[TIMING][Frame {}] UPDATE: removing DropdownShowing from {:?} (closing other)", frame, entity);
                                world.entity_mut(entity).remove::<DropdownShowing>();
                            }
                            info!("[TIMING][Frame {}] UPDATE: adding DropdownShowing to {:?}", frame, *lazy_entity);
                            world.entity_mut(*lazy_entity).insert(DropdownShowing);
                        }
                        info!("[TIMING][Frame {}] UPDATE: on_click handler END for {:?}", frame, *lazy_entity);
                    })),
            )
            .child_signal(
                show.map_true_in(clone!((lazy_entity, opts_for_list) move || {
                    info!("[TIMING] map_true_in: creating Column for {:?}", *lazy_entity);
                    Column::<Node>::new()
                        .with_node(|mut node| {
                            node.width = Val::Percent(100.0);
...
fn main() {
    App::new()
        ...
        .add_systems(Update, |world: &mut World| {
            ...
            let first_showing = world.entity(first).contains::<DropdownShowing>();
            let second_showing = world.entity(second).contains::<DropdownShowing>();
            
            let first_children: Vec<Entity> = world.get::<Children>(first).map(|c| c.iter().collect()).unwrap_or_default();
            let second_children: Vec<Entity> = world.get::<Children>(second).map(|c| c.iter().collect()).unwrap_or_default();
            
            info!("[TIMING][Frame {}] UPDATE START: first_showing={}, second_showing={}, first_children={}, second_children={}", 
                frame, first_showing, second_showing, first_children.len(), second_children.len());
        })
        // Log at end of Update (before PostUpdate)
        .add_systems(Update, |world: &mut World| {
            let frame = world.resource::<FrameCounter>().0;
            let entities = world.resource::<DropdownEntities>();
            let first = *entities.first;
            ...
            info!("[TIMING][Frame {}] UPDATE END: first_showing={}, second_showing={}, first_children={}, second_children={}", 
                frame, first_showing, second_showing, first_children.len(), second_children.len());
        })
        // Log at start of PostUpdate (before signal processing)
        .add_systems(
            PostUpdate,
            (|world: &mut World| {
                let frame = world.resource::<FrameCounter>().0;
                ...
                info!("[TIMING][Frame {}] POSTUPDATE START: first_showing={}, second_showing={}, first_children={}, second_children={}", 
                    frame, first_showing, second_showing, first_children.len(), second_children.len());
            })
            .before(jonmo::SignalProcessing)
        )
        // Log at end of PostUpdate (after signal processing)
        .add_systems(
            PostUpdate,
            (|world: &mut World| {
                let frame = world.resource::<FrameCounter>().0;
                ...
                info!("[TIMING][Frame {}] POSTUPDATE END: first_showing={}, second_showing={}, first_children={}, second_children={}", 
                    frame, first_showing, second_showing, first_children.len(), second_children.len());
            })
            .after(jonmo::SignalProcessing)
        )
